...
class Riskseq(AbstractModel):
    def __init__(self, config, data_feature):
        ...
        self.F_lstm1 = M2LSTM(input_sz=self.Fine_inputs, hidden_sz=self.Fine_hidden, activate="Leaky_relu",
                              num_layers=1)
        self.F_lstm2 = M2LSTM(input_sz=self.Fine_inputs, hidden_sz=self.Fine_hidden, activate="relu", num_layers=1)
        self.C_lstm1 = M2LSTM(input_sz=self.Coarse_inputs, hidden_sz=self.Coarse_hidden, activate="Leaky_relu",
                              num_layers=1)
        self.C_lstm2 = M2LSTM(input_sz=self.Coarse_inputs, hidden_sz=self.Coarse_hidden, activate="Leaky_relu",
                              num_layers=1)
    ...
    def calculate_loss(self, batchsize):
        ...
        for batch in range(batchsize):
            for seq_step in range(6):
                Real = list(
                    np.flatnonzero(
                        (self.Output_Seq[batch, seq_step, :]) > 0))  # 10,6,354 batch_size,time_step,output_dim

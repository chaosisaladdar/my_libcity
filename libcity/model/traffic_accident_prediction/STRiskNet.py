...
class GCN_Layer(nn.Module):
    def __init__(self, num_of_features, num_of_filter):
        ...
        # (batch_size,N,T*D)-->(batch_size,N,num_of_filter)
    ...
    def forward(self, input_, adj):
        ...
        # adj = torch.from_numpy(adj).to(input_.device)
...
class LocalModule(nn.Module):
    ...
    def forward(self, grid_input, target_time_feature):
        ...
        grid_att_score = self.grid_att_softmax(
            F.relu(grid_att_fc1_output + grid_att_fc2_output + self.grid_att_bias))  # (batch_size*W*H,seq_len)
...
class GlobalModule(nn.Module):
    def __init__(self, num_of_graph_feature, nums_of_graph_filters,
        ...
        for idx, num_of_filter in enumerate(nums_of_graph_filters):
            if idx == 0:
                self.road_gcn.append(GCN_Layer(num_of_graph_feature * input_window, num_of_filter))
            else:
                self.road_gcn.append(GCN_Layer(nums_of_graph_filters[idx - 1], num_of_filter))

    def forward(self, graph_feature, road_adj,
        ...
        road_graph_output = graph_feature.view(batch_size, -1, N).permute(0, 2, 1).contiguous()
        ...
        grid_node_map_tmp = grid_node_map \
            ...
            .repeat(batch_size, 1, 1)  # (batch_size,W*H,N)
        graph_output = torch.bmm(grid_node_map_tmp, road_graph_output) \
            .permute(0, 2, 1) \
            .view(batch_size, -1, self.north_south_map, self.west_east_map)
        return graph_output


class _ST_RiskNet(nn.Module):
    def __init__(self, grid_in_channel, num_of_gru_layers,
        ...
        Arguments:
            ...
            west_east_map {int} -- the height of grid data
        """
        super(_ST_RiskNet, self).__init__()
        self.north_south_map = north_south_map
        self.west_east_map = west_east_map

        self.local_module = LocalModule(grid_in_channel, num_of_gru_layers, input_window,
                                        gru_hidden_size, num_of_target_time_feature)

        self.global_module = GlobalModule(num_of_graph_feature, nums_of_graph_filters,
                                          input_window, num_of_gru_layers, gru_hidden_size,
                                          num_of_target_time_feature, north_south_map, west_east_map)

        fusion_channel = 16
        self.grid_weigth = nn.Conv2d(in_channels=gru_hidden_size, out_channels=fusion_channel, kernel_size=1)
        self.graph_weigth = nn.Conv2d(in_channels=nums_of_graph_filters[-1], out_channels=fusion_channel, kernel_size=1)
    ...
    def forward(self, grid_input, target_time_feature, graph_feature,
        ...
        grid_output = self.local_module(grid_input, target_time_feature)
        graph_output = self.global_module(graph_feature, road_adj,
                                          target_time_feature, grid_node_map)
...
class STRiskNet(AbstractTrafficStateModel):
    def __init__(self, config, data_feature):
        super(STRiskNet, self).__init__(config, data_feature)
    ...
    def forward(self, batch):
        ...
        result = self.st_risknet.forward(
